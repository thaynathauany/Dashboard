...
const overviewData = [
	{ name: "Receita", value: "$1,234,567", change: -20, icon: DollarSign },
    ...
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {overviewData.map((item, index) => (
            ...
            <div className="flex items-center justify-between">
                ...
                <div className={`
                  p-3  rounded-full ${item.change >= 0 ? "bg-green-500 bg-opacity-20" : "bg-red-500 bg-opacity-20"}
                `}>
                    <item.icon className={`size-6 ${item.change} >= 0 ? "bg-green-500" : "text-red-500"`} />
                </div>
            </div>

            <div className={`
                mt-4 flez items-center ${item.change >= 0 ? "text-green-500" : "text-red-500"}`}>
